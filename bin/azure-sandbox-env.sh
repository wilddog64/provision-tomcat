...
# Default values - these are the ultimate fallbacks if no other value is found
DEFAULT_AZURE_SUBSCRIPTION_ID=""
DEFAULT_AZURE_TENANT_ID=""
DEFAULT_AZURE_LOCATION="southcentralus" # Default for ACG sandbox
DEFAULT_AZURE_RESOURCE_GROUP=""
DEFAULT_AZURE_VNET_NAME=""
DEFAULT_AZURE_SUBNET_NAME=""
DEFAULT_AZURE_NSG_NAME=""
DEFAULT_AZURE_ADMIN_USERNAME="azureadmin" # Default for ACG Windows VMs
DEFAULT_AZURE_ADMIN_PASSWORD="ChangeM3!SecurePassword" # Default for ACG Windows VMs
DEFAULT_AZURE_VM_NAME="kqvm-win11" # Ensure default for VM name
...
fetch_account_field() {
  ...
  if ! az account show >/dev/null 2>&1; then # Check auth every time for safety
    echo "" # Return empty string on failure or no az
    return 0
  fi
...
# Current values, initialized from env or defaults
AZURE_SUBSCRIPTION_ID="${AZURE_SUBSCRIPTION_ID:-$DEFAULT_AZURE_SUBSCRIPTION_ID}"
AZURE_TENANT_ID="${AZURE_TENANT_ID:-$DEFAULT_AZURE_TENANT_ID}"
...
AZURE_VNET_NAME="${AZURE_VNET_NAME:-$DEFAULT_AZURE_VNET_NAME}"
AZURE_SUBNET_NAME="${AZURE_SUBNET_NAME:-$DEFAULT_AZURE_SUBNET_NAME}"
AZURE_NSG_NAME="${AZURE_NSG_NAME:-$DEFAULT_AZURE_NSG_NAME}"
AZURE_ADMIN_USERNAME="${AZURE_ADMIN_USERNAME:-$DEFAULT_AZURE_ADMIN_USERNAME}"
AZURE_ADMIN_PASSWORD="${AZURE_ADMIN_PASSWORD:-$DEFAULT_AZURE_ADMIN_PASSWORD}"
AZURE_VM_NAME="${AZURE_VM_NAME:-$DEFAULT_AZURE_VM_NAME}"
...
if [[ $AUTO_FILL -eq 1 ]]; then
  ...
  AZURE_SUBSCRIPTION_ID_FETCHED="$(fetch_account_field 'id')"
  AZURE_TENANT_ID_FETCHED="$(fetch_account_field 'tenantId')"

  # Only apply fetched values if not already set by env or CLI arguments
  AZURE_SUBSCRIPTION_ID="${AZURE_SUBSCRIPTION_ID:-$AZURE_SUBSCRIPTION_ID_FETCHED}"
  AZURE_TENANT_ID="${AZURE_TENANT_ID:-$AZURE_TENANT_ID_FETCHED}"
  ...
  if [[ -n "$AZURE_RESOURCE_GROUP" ]]; then
    if [[ -z "$AZURE_VNET_NAME" ]]; then
      AZURE_VNET_NAME="$(az network vnet list --resource-group "$AZURE_RESOURCE_GROUP" --query "[0].name" -o tsv 2>/dev/null | head -n 1 || echo "$AZURE_VNET_NAME")"
    ...
    if [[ -z "$AZURE_NSG_NAME" ]]; then
      AZURE_NSG_NAME="$(az network nsg list --resource-group "$AZURE_RESOURCE_GROUP" --query "[0].name" -o tsv 2>/dev/null | head -n 1 || echo "$AZURE_NSG_NAME")"
